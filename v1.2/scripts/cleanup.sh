...
check_prerequisites() {
    ...
    local optional_cmds=(conda npm sqlite3)
...
cleanup_temp_files() {
    ...
    find "${BASE_DIR}/mlflow/artifacts" -type f -mtime +30 -delete 2>/dev/null || true
    find /tmp -type f -name "mlflow-*" -mtime +1 -delete 2>/dev/null || true
...
backup_data() {
    ...
    # Backup MLflow database if sqlite3 is available
    if command -v sqlite3 &> /dev/null; then
        if [[ -f "${BASE_DIR}/mlflow/db/mlflow.db" ]]; then
            log "Backing up MLflow database..."
            sqlite3 "${BASE_DIR}/mlflow/db/mlflow.db" ".backup '${backup_dir}/mlflow.db'"
            verify_backup "${backup_dir}/mlflow.db"
        fi
    else
        warn "sqlite3 not installed - skipping MLflow database backup"
    fi
...
optimize_storage() {
    ...
    # Remove old MLflow runs if sqlite3 is available
    if command -v sqlite3 &> /dev/null; then
        if [[ -f "${BASE_DIR}/mlflow/db/mlflow.db" ]]; then
            log "Cleaning old MLflow runs..."
            sqlite3 "${BASE_DIR}/mlflow/db/mlflow.db" "DELETE FROM runs WHERE end_time < datetime('now', '-30 days');" 2>/dev/null || true
            sqlite3 "${BASE_DIR}/mlflow/db/mlflow.db" "VACUUM;" 2>/dev/null || true
        fi
    else
        warn "sqlite3 not installed - skipping MLflow database optimization"
    fi
